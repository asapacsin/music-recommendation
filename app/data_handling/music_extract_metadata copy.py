...
Runs always load the existing JSON first, merge with files on disk (union of paths), and write back
that merged set — JSON-only rows (e.g. file temporarily missing) are preserved.

Incremental (default): only call the API for paths not yet in the JSON, or (with --confidence)
those below the threshold. Does not drop existing rows.

--rebuild: re-extract via API for all paths in the union (or only low-confidence rows if
--confidence is set). Use this for a full refresh.

Optional: --confidence THRESHOLD — only call the API for rows with stored confidence < THRESHOLD
(e.g. 0.35); rows at or above the threshold are left as-is.

Optional: --reasoning — use Grok reasoning model (see XAI_REASONING_MODEL / default grok-4.20-reasoning).
"""
...
def _stored_confidence(row: dict[str, Any]) -> float | None:
    """Return stored confidence from a record, or None if missing/invalid."""
...
def _needs_refetch_for_confidence_threshold(
    audio: str,
    existing_by_filename: dict[str, dict[str, Any]],
    threshold: float,
) -> bool:
    """
    True if we should call the API for this path when --confidence THRESHOLD is set.

    Refetch when there is no row, confidence is missing, or confidence is strictly below threshold.
    """
    row = existing_by_filename.get(audio)
    if row is None:
        return True
    c = _stored_confidence(row)
    if c is None:
        return True
    return c < threshold
...
def resolve_xai_model(args: argparse.Namespace) -> str:
    """
    Effective model: explicit --model wins; else --reasoning uses XAI_REASONING_MODEL or
    DEFAULT_REASONING_MODEL; else XAI_MODEL or DEFAULT_MODEL.
    """
    if getattr(args, "model", None) is not None:
        return args.model
    if getattr(args, "reasoning", False):
        return os.getenv("XAI_REASONING_MODEL", DEFAULT_REASONING_MODEL)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Extract music metadata (audio, text, mood, confidence): merge existing JSON with "
            "music-dir, update incrementally unless --rebuild."
        )
    )
    parser.add_argument(
        "--music-dir",
    ...
    parser.add_argument(
        ...
        metavar="NAME",
        help=(
            f"Override xAI model id. If omitted: use --reasoning mode or env XAI_MODEL / "
            f"{DEFAULT_MODEL} (non-reasoning)."
        ),
    )
    parser.add_argument(
        "--reasoning",
        action="store_true",
        help=(
            f"Use Grok reasoning model (env XAI_REASONING_MODEL or {DEFAULT_REASONING_MODEL}). "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Full re-extract: call the API for (almost) every path in the union of disk + existing JSON "
            "(use --confidence to limit to low-confidence rows only). Default incremental mode only "
            "adds/updates rows; it preserves the full merged JSON."
        ),
    )
    parser.add_argument(
        "--confidence",
        ...
        metavar="THRESHOLD",
        help=(
            "Only call the API for paths missing from the JSON or whose stored confidence is "
            "strictly below THRESHOLD (e.g. 0.35), evaluated over the union of disk + existing JSON. "
            "With --rebuild, same filter limits which paths are re-extracted."
        ),
    )
    ...
    model_name = resolve_xai_model(args)
    logging.info("xAI model: %s", model_name)
    ...
    if args.confidence is not None:
        if not 0.0 < args.confidence <= 1.0:
            raise ValueError("--confidence THRESHOLD must be in (0, 1], e.g. 0.35")

    # Always merge disk scan with existing JSON so we never drop JSON-only rows on incremental runs.
    all_filenames = sorted(set(filenames_disk) | set(existing_by_filename.keys()))
    if args.limit is not None:
        all_filenames = all_filenames[: args.limit]
    logging.info(
        "Working set: %d paths (union of music-dir + existing JSON%s)",
        len(all_filenames),
        f", first {args.limit} only" if args.limit is not None else "",
    )
    ...
    if args.rebuild:
        if args.confidence is not None:
            missing_filenames = [
                ...
                if _needs_refetch_for_confidence_threshold(
                    name, existing_by_filename, args.confidence
                )
            ]
            skipped = len(all_filenames) - len(missing_filenames)
            logging.info(
                "Rebuild + --confidence %s: API for %d paths (skipped %d with confidence >= %s)",
                args.confidence,
            ...
            logging.info(
                "Rebuild mode: full re-extract via API for %d paths",
                len(missing_filenames),
            )
    else:
        if args.confidence is not None:
            ...
            logging.info(
                "Incremental + --confidence %s: %d paths need API",
                args.confidence,
                len(missing_filenames),
            )
        else:
            missing_filenames = [name for name in all_filenames if name not in existing_by_filename]
            logging.info(
                "Incremental mode: %d paths already in JSON, %d need API (new or missing rows)",
